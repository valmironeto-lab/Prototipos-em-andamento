...
class BSM_DB {
	...
	public static function create_database_tables() {
        ...
        // MODIFICAÇÃO: A estrutura da tabela de automações agora usa JSON para as configurações do gatilho
        $sql_automations = "CREATE TABLE {$wpdb->prefix}bluesendmail_automations ( automation_id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT, name varchar(255) NOT NULL, status varchar(20) NOT NULL DEFAULT 'inactive', trigger_type varchar(50) NOT NULL, trigger_settings longtext, created_at datetime NOT NULL DEFAULT '0000-00-00 00:00:00', PRIMARY KEY  (automation_id), KEY trigger_type (trigger_type), KEY status (status) ) $charset_collate;";
        
        // MODIFICAÇÃO: Tabela de fila de automação simplificada para o novo motor
        $sql_automation_queue = "CREATE TABLE {$wpdb->prefix}bluesendmail_automation_queue ( queue_id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT, automation_id bigint(20) UNSIGNED NOT NULL, contact_id bigint(20) UNSIGNED NOT NULL, status varchar(20) NOT NULL DEFAULT 'waiting', next_step_index int(11) NOT NULL DEFAULT 0, data_layer longtext, process_at datetime NOT NULL DEFAULT '0000-00-00 00:00:00', created_at datetime NOT NULL DEFAULT '0000-00-00 00:00:00', PRIMARY KEY  (queue_id), KEY automation_id (automation_id), KEY contact_id (contact_id), KEY status (status) ) $charset_collate;";
		
        // MODIFICAÇÃO: Reintroduzindo parent_step_id e branch para condicionais
        $sql_automation_steps = "CREATE TABLE {$wpdb->prefix}bluesendmail_automation_steps ( step_id bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT, automation_id bigint(20) UNSIGNED NOT NULL, parent_step_id bigint(20) UNSIGNED NOT NULL DEFAULT 0, branch varchar(10) DEFAULT NULL, step_order int(11) NOT NULL DEFAULT 0, step_type varchar(50) NOT NULL, step_settings longtext, PRIMARY KEY  (step_id), KEY automation_id (automation_id), KEY parent_step_id (parent_step_id) ) $charset_collate;";
    ...
    private static function verify_and_add_columns() {
        ...
        // Verifica a coluna 'fields' na tabela de formulários
        ...
        // Verifica a coluna 'button_color' na tabela de formulários
        ...
        // Verifica a coluna 'phone' na tabela de contatos
