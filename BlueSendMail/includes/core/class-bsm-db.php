...
class BSM_DB {
	...
	public static function create_database_tables() {
		...
		// ETAPA 1: Adicionada a coluna 'attempts'
    ...
    private static function verify_and_add_columns() {
        ...
        $table_automation_queue = $wpdb->prefix . 'bluesendmail_automation_queue';
        ...
        // ETAPA 1: Garante que a coluna 'attempts' seja adicionada em atualizações do plugin
        $column_attempts_exists = $wpdb->get_var( $wpdb->prepare( "SHOW COLUMNS FROM `{$table_automation_queue}` LIKE %s", 'attempts' ) );
        if ( empty( $column_attempts_exists ) ) {
            $wpdb->query( "ALTER TABLE `{$table_automation_queue}` ADD `attempts` tinyint(1) UNSIGNED NOT NULL DEFAULT 0 AFTER `data_layer`;" );
        }
    }

    /**
     * Busca uma automação completa (com seus passos) pelo ID.
     *
     * @param int $automation_id O ID da automação.
     * @return stdClass|null O objeto da automação ou nulo se não for encontrado.
     */
    public function get_automation_by_id( $automation_id ) {
        global $wpdb;
        ...
        // Busca os dados principais da automação
        ...
        // Desserializa as configurações do gatilho
        $automation->trigger_settings = maybe_unserialize( $automation->trigger_settings );

        // Busca os passos da automação
        $automation->steps = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}bluesendmail_automation_steps WHERE automation_id = %d ORDER BY step_order ASC", $automation_id ) );

        // Desserializa as configurações de cada passo
        if ( ! empty( $automation->steps ) ) {
            foreach ( $automation->steps as &$step ) { // Usa referência (&) para modificar no próprio array
