...
class BSM_Cron {
	...
	/**
     * Processa a fila de automação de acordo com a nova arquitetura (Fase 3).
     */
		...
		foreach ( $items_to_process as $item ) {
			...
			if ( $action ) {
				...
				try {
					...
					$this->plugin->log_event( 'error', 'automation_engine', "Erro ao executar a ação '{$step->action_id}' para o item da fila #{$item->queue_id}: " . $e->getMessage() );
					$wpdb->update($table_queue, ['status' => 'failed'], ['queue_id' => $item->queue_id]);
				}
			} else {
