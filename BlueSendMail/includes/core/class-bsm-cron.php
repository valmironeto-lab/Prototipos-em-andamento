...
define( 'BSM_AUTOMATION_MAX_RETRIES', 3 ); // ETAPA 1: Constante para o número de tentativas
define( 'BSM_AUTOMATION_RETRY_INTERVAL', 15 * MINUTE_IN_SECONDS ); // ETAPA 1: Constante para o intervalo de novas tentativas
...
class BSM_Cron {
	...
	/**
     * Processa a fila de automação com sistema de tentativas (retries).
     */
	public function process_automation_queue() {
		...
		$table_steps = $wpdb->prefix . 'bluesendmail_automation_steps';

		// Busca itens na fila que estão prontos para serem processados
		...
		// Marca os itens como 'processing' para evitar processamento duplicado
		...
		foreach ( $items_to_process as $item ) {
			// Busca os detalhes do passo a ser executado
			$step = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table_steps} WHERE step_id = %d", $item->step_id ) );

			if ( ! $step ) {
				$this->plugin->log_event( 'error', 'automation_engine', "Passo #{$item->step_id} não encontrado para o item da fila #{$item->queue_id}. O fluxo foi interrompido." );
				$wpdb->update($table_queue, ['status' => 'failed'], ['queue_id' => $item->queue_id]);
				continue;
			}

			// Usa o Action Registry para obter a instância da ação correta
			$action = BSM_Actions_Registry::get($step->action_id);

			if ( $action ) {
				$data_layer = new BSM_Data_Layer( maybe_unserialize( $item->data_layer ) );
				$settings   = maybe_unserialize( $step->step_settings );
				
				// ETAPA 1: Implementação do Try/Catch com lógica de tentativas
				try {
					$action->run( $item, $data_layer, $settings );
				} catch (Exception $e) {
					$error_message = sprintf(
						"Erro ao executar a ação '%s' para o item da fila #%d: %s",
						$step->action_id, $item->queue_id, $e->getMessage()
					);
					$this->plugin->log_event('error', 'automation_engine', $error_message);

					$attempts = (int) $item->attempts + 1;

					if ($attempts >= BSM_AUTOMATION_MAX_RETRIES) {
						// Excedeu o limite, marca como falha permanente
						$wpdb->update($table_queue, ['status' => 'failed', 'attempts' => $attempts], ['queue_id' => $item->queue_id]);
						$this->plugin->log_event('error', 'automation_engine', "O item da fila #{$item->queue_id} falhou permanentemente após {$attempts} tentativas.");
					} else {
						// Tenta novamente mais tarde
						$next_try_timestamp = time() + BSM_AUTOMATION_RETRY_INTERVAL;
						$next_try_gmt = gmdate('Y-m-d H:i:s', $next_try_timestamp);
						$wpdb->update(
							$table_queue,
							['status' => 'waiting', 'attempts' => $attempts, 'process_at' => $next_try_gmt],
							['queue_id' => $item->queue_id]
						);
						$this->plugin->log_event('warning', 'automation_engine', "O item da fila #{$item->queue_id} será processado novamente em " . BSM_AUTOMATION_RETRY_INTERVAL / 60 . " minutos (Tentativa {$attempts}/" . BSM_AUTOMATION_MAX_RETRIES . ").");
					}
				}
			} else {
				$this->plugin->log_event( 'error', 'automation_engine', "Classe de ação '{$step->action_id}' não encontrada para o passo #{$step->step_id}. O fluxo foi interrompido." );
				$wpdb->update($table_queue, ['status' => 'failed'], ['queue_id' => $item->queue_id]);
			}
		}
