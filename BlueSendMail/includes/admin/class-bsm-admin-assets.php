...
class BSM_Admin_Assets {
    ...
    public function enqueue_assets( $hook ) {
		...
		if ( $is_automation_editor ) {
            ...
            // Busca e agrupa os gatilhos
            $triggers = BSM_Triggers_Registry::get_all();
            foreach ($triggers as $trigger) {
                if(is_object($trigger)) {
                    $definitions['triggers'][$trigger->group][] = ['id' => $trigger->id, 'name' => $trigger->name];
                }
            }

            // Busca e agrupa as ações
            $actions = BSM_Actions_Registry::get_all();
            foreach ($actions as $action) {
                if(is_object($action)) {
                     $definitions['actions'][$action->group][] = ['id' => $action->id, 'name' => $action->name];
                }
            }
            ...
            $script_data = [
                ...
                'saved_trigger_id' => $automation->trigger_id ?? '',
                'saved_trigger_settings' => $automation->trigger_settings ?? new stdClass(),
                'saved_steps' => $automation->steps ?? [],
            ];
