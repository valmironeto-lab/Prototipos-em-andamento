    ...
    public function enqueue_assets( $hook ) {
		...
		if ( $is_automation_editor ) {
            ...
            $script_data = array( 
                ...
                'automation_nonce' => wp_create_nonce('bsm_automation_nonce')
            );
            
            wp_localize_script( 'bsm-automation-builder', 'bsm_admin_data', $script_data );
		}
		
		$general_script_data = [];

		if ( $is_dashboard_page ) {
			$general_script_data['growth_chart_data'] = $this->get_contacts_growth_data();
			$general_script_data['performance_chart_data'] = $this->get_performance_data();
		}
		...
		if ( $is_reports_page && ! empty( $_GET['campaign_id'] ) ) {
			...
			$general_script_data['chart_data'] = array( 'sent' => (int) $sent, 'opens' => (int) $unique_opens, 'clicks' => (int) $unique_clicks, 'labels' => array( 'not_opened' => __( 'Não Aberto', 'bluesendmail' ), 'opened' => __( 'Abertura Única', 'bluesendmail' ), 'clicked' => __( 'Clique Único (dentro dos que abriram)', 'bluesendmail' ) ) );
		}

        if ( !empty($general_script_data) ) {
            wp_localize_script( 'bluesendmail-admin-script', 'bsm_general_data', $general_script_data );
        }
