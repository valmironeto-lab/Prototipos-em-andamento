...
class Assets {
    ...
    public function enqueue_assets( $hook ) {
		...
		// Mapeamento de páginas para otimização
		$is_dashboard_page = 'bluesendmail' === $page;
		$is_campaign_editor = 'bluesendmail-new-campaign' === $page || ( 'bluesendmail-campaigns' === $page && 'edit' === $action );
        $is_template_editor = 'bluesendmail-templates' === $page && in_array($action, ['new', 'edit']);
        ...
        $is_form_editor = ('bluesendmail-forms' === $page && in_array($action, ['new', 'edit']));

		$main_script_dependencies = array( 'jquery' );
        $script_data = ['ajax_url' => admin_url('admin-ajax.php')];

		// Carrega Chart.js apenas no Dashboard e Relatórios
		if ( $is_dashboard_page || $is_reports_page ) {
            ...
            $script_data['is_dashboard_page'] = $is_dashboard_page;
            $script_data['is_reports_page'] = $is_reports_page;
		}
		
		// Carrega Select2 apenas no editor de campanha e na importação
		if ( $is_campaign_editor || $is_import_page ) {
            ...
            $script_data['is_import_page'] = $is_import_page;
		}

        // Carrega o editor de formulário e color picker apenas na página de edição de formulários
        if ( $is_form_editor ) {
		...
		// Carrega o editor de campanha/template (WP Editor, merge tags, etc)
		if ( $is_campaign_editor || $is_template_editor ) { 
            $main_script_dependencies[] = 'wp-editor';
            $script_data['is_campaign_editor'] = true; // Flag para a lógica JS
            $script_data['nonce'] = wp_create_nonce('bsm-template-nonce');
        }

		// Carrega o construtor de automação apenas na sua página
		...
		// Carrega o script de administração principal em todas as páginas do plugin
		...
		// Adiciona dados localizados se houver dados a serem passados
