...
define( 'BLUESENDMAIL_VERSION', '2.3.0' );
...
final class BlueSendMail {
	...
	private function handle_click_tracking() {
		...
		if ( $queue_item ) {
            $wpdb->insert( "{$wpdb->prefix}bluesendmail_email_clicks", array( 'queue_id' => $queue_id, 'campaign_id' => $queue_item->campaign_id, 'contact_id' => $queue_item->contact_id, 'original_url' => $original_url, 'ip_address' => $_SERVER['REMOTE_ADDR'] ?? '', 'user_agent' => $_SERVER['HTTP_USER_AGENT'] ?? '' ) );
            
            // Dispara o gatilho de automação
            $contact = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}bluesendmail_contacts WHERE contact_id = %d", $queue_item->contact_id ) );
            $campaign = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}bluesendmail_campaigns WHERE campaign_id = %d", $queue_item->campaign_id ) );
            if ($contact && $campaign) {
                $data_layer = new BSM_Data_Layer([
                    'contact' => $contact,
                    'campaign' => $campaign,
                    'link' => (object)['url' => $original_url]
                ]);
                do_action('bsm_trigger_event', 'contact_clicked_link', $data_layer);
            }
        }
	...
	private function handle_tracking_pixel() {
        ...
        if ($queue_item) {
            ...
            if ($contact && $campaign) {
                ...
                do_action('bsm_trigger_event', 'contact_opened_campaign', $data_layer);
            }
        }
